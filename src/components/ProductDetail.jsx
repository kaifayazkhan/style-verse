...
const ProductDetail = ({ ...data }) => {
  ...
  return (
    <>
      ...
      >
        <div className="w-full md:w-[40%] bg-gray-100 relative">
          <img
            src={image_url}
            alt={product_name}
            className="w-full h-60 md:h-72 md:max-h-80 object-contain mix-blend-multiply"
          />
        ...
        <div className="w-full md:w-[50%] flex flex-col gap-3 ">
          ...
          <h2 className={`${style["heading-medium"]} uppercase`}>
            {product_name}
          </h2>
          <div className={`${style["flex-row-start"]} gap-2`}>
            <p className={`${style["heading-xsmall"]} line-through`}>
              ${price}
            </p>
          ...
          {state?.cart?.find(
            (item) =>
              ...
              item.product_id === `${product_id}${productSize}`
          ) ? (
            <button className={`${style["btn-outline"]}`}>
              <Link to="/cart">Go to Cart</Link>
            </button>
          ) : (
            <button onClick={addToCart} className={`${style["global-btn"]}`}>
              Add to Cart
            </button>
          )}
        </div>
      ...
      {openModal && (
        <div className="fixed left-0 top-0 right-0 bottom-0  bg-[#0009] backdrop-blur-sm  flex justify-center items-center p-[5%]">
